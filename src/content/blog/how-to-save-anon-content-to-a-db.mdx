...
# heroImageCredit: [https://unsplash.com/@dtravisphd, David Travis]
# status: published
---
...
4. **Redirect to our new note and delete the local content:** Finally, after redirecting to the new note page, we also include a param that will enable deleting the localStorage content, since it is now safely in the database.
...
On the note detail view, we check for this param and use it to safely clear local storage, knowing that it has been saved to the database. And with that we are done <Emoji symbol="🎉" label="celebrate emoji" />.
