...
pubDate: 09-02-2023
heroImage: '/images/blog/how-to-save-anon-content-to-a-db/featured.jpg'
toc: true
---
...
2. `cy.get('[data-test="emailInput...` - this is where we type in the test email address.
3. `cy.get('[data-test="formSubmit"]').click();` - here, we click the form submit button and, which results in an API call, that then is intercepted as per step 1 above.
4. `cy.get('[data-test="successMessage"]')...` - assuming our form submit was successful, we expect a success response from the server.
